...
const fetchPopulares = async (filtro = "movie", pagina = 1) => {
  const tipo = filtro === "movie" ? "movie" : "tv";
  const url = `https://api.themoviedb.org/3/${tipo}/popular?api_key=d43da283da70651ed97d5a56b4db9d6b&language=es-MX&region=US&page=${pagina}`;
...
const fetchBusqueda = async (pagina = 1) => {
  ...
  let url;
  if (tipo === "movie") {
    url = `https://api.themoviedb.org/3/discover/movie?api_key=d43da283da70651ed97d5a56b4db9d6b&language=es-MX&sort_by=popularity.desc&include_adult=false&page=1&with_genres=${idGenero}&primary_release_date.gte=${añoInicial}-01-01&primary_release_date.lte=${añoFinal}-12-31&region=US&page=${pagina}`;
  } else if (tipo === "tv") {
    url = `https://api.themoviedb.org/3/discover/tv?api_key=d43da283da70651ed97d5a56b4db9d6b&language=es-MX&sort_by=popularity.desc&include_adult=false&page=1&with_genres=${idGenero}&first_air_date.gte=${añoInicial}-01-01&first_air_date.lte=${añoFinal}-12-31&region=US&page=${pagina}`;
  }
